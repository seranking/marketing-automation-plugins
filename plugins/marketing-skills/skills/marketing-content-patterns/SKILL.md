---
name: marketing-content-patterns
description: >
  Analyze top-performing posts across three performance lenses — engagement, impressions, and
  engagement rate — to identify content patterns, then validate every pattern against real
  search and question demand (SE Ranking) so the hypotheses target BOTH channels: what to post
  more of, and which winning social topics deserve a page or article because demand exists
  beyond the feed. Use when the user asks "what's working in my content", "what should I post
...
Look at what's actually working — then tell the user why, what to try next, and **where else the winning topics can win**: every named pattern is checked against live search/question demand, so a topic that resonates socially AND has search volume gets routed to site content too, not just more posts.

This skill analyzes top posts across three performance lenses, finds patterns in the content itself (format, hook, topic, tone, structure), validates them against demand, and translates them into specific, testable hypotheses for future posts and pages.

**This is not a performance report.** It's a content brief grounded in data from both sides of the funnel.
...
### Step 3.5: Validate patterns against demand (signature step)

For each named pattern's topic/theme (not its format — formats are audience behavior, topics are demand):

- `DATA_getKeywordsMetrics` on the topic's core phrase — volume, difficulty.
- `DATA_getKeywordQuestions` on 1–2 seeds — are people asking about this beyond the feed?

Classify each pattern: **audience + demand** (resonates socially AND has search/question demand → recommend site content too — a page or article via `marketing-content-brief`, which ships with its own distribution), **audience-only** (feed magic without search demand → keep it social), **unknown** (SE Ranking returned nulls — say so, don't guess).

Sequential calls, ~10 req/s; this adds roughly 4–8 quick calls total. If the SE Ranking MCP isn't connected, skip with a visible note — the analysis still ships, minus the routing.
...
**Make a page for:**
- [Pattern with audience + demand: "{topic} — {volume}/mo + {n} live questions → marketing-content-brief" — skip the line if none qualified]
...
Search demand: [{volume}/mo + {n} questions | audience-only — no search demand found | unknown]
➡️ Try this: [Specific, actionable thing to test in the next post or batch — and "make a page" when demand backs it]
