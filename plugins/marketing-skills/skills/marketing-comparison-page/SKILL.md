---
name: marketing-comparison-page
description: >
  Generate and LAUNCH an SEO-optimized "X vs Y" comparison or "alternatives to X" page. Produces
  the balanced page draft with feature matrix, schema and conversion blocks (SE Ranking data) —
  then, unlike a page generator, runs the launch: the copy goes onto a Planable Universal content
  page for comments and formal approval, and a supporting "X vs Y" social series is drafted and
  labeled so the page ships with distribution. Use when the user asks for a "comparison page",
  "vs page", "alternatives page", "X vs Y", "alternative to X", "comparison page with approval",
  or wants comparison content created, reviewed, and promoted in one flow. Requires both the
  SE Ranking and Planable MCPs. For the page draft alone without the approval flow and social
  series, use seo-competitor-pages from the seo-skills plugin instead.
---

# Comparison page launch

Produce conversion-tuned landing pages targeting comparative-intent keywords ("X vs Y", "alternatives to X", "best X for Y") — and launch them: the draft goes through a real review/approval surface in Planable, and a supporting social series ships with it. The deliverable is a paste-ready page draft with feature matrix, balanced verdict, schema markup, a CTA flow, an approval trail, and labeled distribution drafts.
...
- **Planable MCP** connected — a Universal content page for the approval flow and social pages for the series (pages are added inside Planable; if the Universal page is missing, ask the user to add one or skip the approval layer with a note).
- Claude's `WebFetch` tool available.
- User provides: (a) the user's brand/product (the page's hero), (b) target competitor(s) — at least one, optionally up to 5 for an alternatives page, (c) page type (auto-detected from the keyword if user doesn't specify), (d) target country (default `us`), (e) the Planable workspace.
...
9. **Launch layer (signature step)**
   - **Approval surface:** create the page copy as a post on the Planable **Universal content page** (`create_post(workspaceId, universalPageId, text)`), with the intended live URL in the body. Reviewers use `create_comment` (`teamOnly: true` for internal notes), revisions go in via `update_post`, sign-off via `approve_post` (needs a configured approval workflow; otherwise comments serve as sign-off — say which applies). Don't group the page copy with social posts — universal and social pages split into different sub-groups.
   - **Supporting social series:** draft 3–5 posts from the page's strongest material — the verdict as a hot take, the feature matrix as a carousel outline, the top PAA answer as a question-hook post. Create them on the social pages, label everything `mkt:vs-<slug>` (`list_labels` / `create_label`).
   - **Scheduling — ask:** offer to spread the series across the week after the page's planned publish date, or leave undated. Drafts only — never set `publishAtScheduledDate`.
   - Publishing the approved copy to the website/CMS stays manual — state the handoff. After launch, `marketing-campaign-impact` measures the label; seo-skills' `seo-page` tracks the URL's ranking trajectory.

## Output format

Create a folder `marketing-comparison-page-{target-slug}-{YYYYMMDD}/` with:

```
marketing-comparison-page-{target-slug}-{YYYYMMDD}/
...
- Run seo-skills' `seo-page` on the published page after 30 days to track ranking trajectory; `marketing-campaign-impact` measures the social series via its label.
- Pair with seo-skills' `seo-content-audit` to E-E-A-T-check the draft before publishing.
