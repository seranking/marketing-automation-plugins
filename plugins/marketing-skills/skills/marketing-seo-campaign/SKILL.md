---
name: marketing-seo-campaign
description: >
  Turn SE Ranking search-opportunity insights into a measurable social campaign in Planable.
  Use whenever the user wants SEO research converted into social content — keyword gaps, competitor
  wins, ranking losses, question keywords — with posts drafted and scheduled in Planable. Trigger on
  "what should we post based on our SEO data", "turn these keyword gaps into posts", "competitor ranks
  for X, make social content about it", "build a social campaign from search demand", "fill the
  calendar based on what people search". Always activate when SE Ranking research is the starting
  point and Planable is the destination. Every batch is instrumented — label `mkt:{slug}` plus a
  campaign passport with baseline numbers — so marketing-campaign-impact can later prove whether it
  worked. Requires both MCPs. Run marketing-gap-matrix first to decide which gaps deserve a campaign;
  this skill executes the social route.
---
...
Report how many drafts were created, in which workspace/pages, the proposed schedule, the campaign label + passport location, and any `validationErrors` (e.g. Instagram needs media — flag it). Close with: *"In 2–8 weeks, run `marketing-campaign-impact` to see whether the campaign moved the numbers — the passport has everything it needs."* For a period-wide view across channels, `marketing-report` includes a per-campaign section.
