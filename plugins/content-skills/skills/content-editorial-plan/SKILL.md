---
name: content-editorial-plan
description: Build a quarter-ready editorial plan from seed keywords — intent-grouped clusters with pillar+spoke architecture, then map every spoke onto a 12-week publication calendar with cadence, priorities, a brief queue, and per-month social themes pulled from Planable engagement data when connected. Use whenever the user wants a content plan they can actually execute — "editorial plan", "quarterly content plan", "content calendar from keywords", "plan our next quarter of content", "what should we publish and when". Requires the SE Ranking MCP; the Planable MCP is optional for social themes. For keyword-cluster architecture alone without a calendar, use seo-keyword-cluster from seranking/seo-skills instead.
---

> Example output: [examples/seo-keyword-cluster-headless-cms-20260514/PLAN.md](https://github.com/seranking/seo-skills/blob/main/examples/seo-keyword-cluster-headless-cms-20260514/PLAN.md) (hosted in the seranking/seo-skills repo)
...
7. **Quality scorecard** (post-synthesis validation)
   ...
   - **Output.** If all four metrics pass, append a single line to `EDITORIAL-PLAN.md` under "## Quality scorecard": `All gates passed (cannibalisation/orphan/coverage/anchor-diversity).` If **any** metric fails, append a "## Quality scorecard" section to `EDITORIAL-PLAN.md` with red/yellow/green rows for each metric (red = fail, yellow = within 10% of threshold, green = pass), and annotate the verdict header at the top of `EDITORIAL-PLAN.md` with `(needs review — N quality-gate failures)`. Also write the same scorecard verbatim to `06-quality-scorecard.md` in the output folder so it's auditable independently.

## Output format

Create a folder `content-editorial-plan-{target-slug}-{YYYYMMDD}/` with:

```
content-editorial-plan-{target-slug}-{YYYYMMDD}/
...
`EDITORIAL-PLAN.md` follows this shape:
...
## Phase 2: From cluster to calendar (what makes it an editorial plan)

A cluster diagram doesn't ship content — a calendar with cadence and next actions does.

1. **12-week mapping.** Distribute the pillar + spokes across the next 12 weeks using the build order from the cluster analysis. Default cadence: pillar first (weeks 1–2), then spokes by priority. Ask the user for their real capacity (pieces per week) and adjust — an honest calendar beats an ambitious one. For each slot record: week, working title, target keyword, intent, funnel stage, estimated effort (S/M/L).
2. **Brief queue.** For the first 3 planned pieces, add a one-line brief seed (angle + must-cover points from the cluster data) so `content-campaign-brief` can pick them up next without re-research.
3. **Social themes per month (optional).** If the Planable MCP is connected, pull recent top-performing posts for the workspace and note 1–2 recurring engagement themes per month next to the publication slots they support — so the social calendar reinforces the blog calendar instead of running parallel to it.

## Final output (replaces the standalone cluster plan)

**EDITORIAL-PLAN.md**: the Phase 1 cluster architecture, then a Quarter calendar table (12 weeks), then the Brief queue, then monthly social themes.
