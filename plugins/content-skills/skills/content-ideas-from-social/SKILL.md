---
name: content-ideas-from-social
description: Turn what already works on social into a search-validated content plan. Finds recurring topics in the client's top-performing Planable posts, validates each theme with SE Ranking keyword data (volume, difficulty, question keywords), and delivers a mini content plan that connects proven audience engagement to real search demand. Use whenever the user wants content ideas grounded in evidence — "what should we write based on our social data", "turn our top posts into SEO topics", "content ideas from what performs", "our audience loves X, is there search demand for it", "what blog posts should we write next". Requires the Planable and SE Ranking MCPs. For social pattern analysis alone, without keyword validation or a content plan, use content-pattern-intelligence from Planable/smm-skills instead, when that plugin is installed.
---
...
After delivering the in-chat text analysis, generate a self-contained HTML file saved to the working folder as `[client]-content-patterns-[month].html` and share it with the user.
...
## Phase 2: Search validation → content plan (the SEO bridge)

Patterns tell you what resonates with the existing audience; this phase proves whether that interest exists in search — and turns the overlap into a plan. This is the difference between "post more of X" and "X is worth a content investment".

1. From the pattern analysis above, list the 3–5 strongest recurring topics.
2. For each topic, pull SE Ranking data: `DATA_getKeywordsMetrics` (volume/difficulty for the head term), `DATA_getRelatedKeywords`, and `DATA_getKeywordQuestions`.
3. Classify each topic: **strong search demand** (build SEO content), **social-only interest** (keep as a social series — still valuable, different channel), **no demand** (drop, or treat as a brand-awareness play with eyes open).
4. For topics with demand, produce a mini content plan: working title, target keyword, intent, suggested format (derived from the intent), and which winning social pattern the piece should borrow its angle from.

## Final output (extends the pattern report)

**CONTENT-IDEAS.md**: the pattern summary from Phase 1, then a Validation table (topic, social evidence, search volume, KD, classification), then the mini content plan — 3–5 pieces ready to hand to `content-campaign-brief` for full briefing.
