...
3. Optional: the final `GAP-SPRINT.md` (see Final output below) is the document to circulate.
...
If `brand-profile.md` exists in the working folder (created by `content-brand-setup`), read it before writing anything and apply its voice, banned words, audience notes, and CTA conventions to every draft and brief this skill produces. If it doesn't exist, proceed with a neutral professional tone and suggest running `content-brand-setup` once at the end.

## Phase 2: Article briefs for the top gaps (beyond posts)

Social posts intercept demand this week; articles capture it durably. A gap worth a campaign is usually a gap worth a page.

1. **Pick the top 2–3 gaps** from the research above, by search volume × winnability (achievable difficulty, weak current SERP).
2. **Produce a light article brief for each**: working title, target keyword + 2–3 secondaries, searcher intent, the competitor angle to beat (from the SERP data already pulled — no re-research), a 5–7 item H2 outline, internal-link candidates, and which posts from the campaign above will support the article at launch.
3. **Sequence everything into one sprint plan**: week 1 — the social campaign intercepts demand; weeks 2–4 — article production; publish week — reuse the supporting posts as launch distribution. One timeline, both channels.

## Final output (extends the social campaign)

**GAP-SPRINT.md**: the campaign summary from Phase 1 (posts live in Planable as drafts), then per-gap article briefs, then the sprint timeline. Each brief is ready for `content-campaign-brief` to deepen into a full campaign brief if the user wants.
