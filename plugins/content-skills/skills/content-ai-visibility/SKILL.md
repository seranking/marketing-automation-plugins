---
name: content-ai-visibility
description: Improve how a brand shows up in AI answers with a two-front plan — owned pages and social. Measures AI-search visibility across ChatGPT, Perplexity, Gemini, AI Overview and AI Mode via SE Ranking, clusters owned/contested/missing prompts, then delivers BOTH passage-level citability recommendations for existing pages (the fixes that make LLMs cite you) AND social drafts in Planable targeting the missing narratives, with a before/after share-of-voice re-check. Use whenever the goal is AI visibility through content — "why doesn't ChatGPT mention us", "AEO/GEO plan", "get cited by AI", "AI visibility recovery", "we're invisible in AI answers", "improve AI Overview presence". Requires the SE Ranking and Planable MCPs. For the social-campaign-only version without on-page recommendations, use ai-search-gaps-to-social-campaign from seranking/seo-skills instead.
---
...
If `brand-profile.md` exists in the working folder (created by `content-brand-setup`), read it before writing anything and apply its voice, banned words, audience notes, and CTA conventions to every draft and recommendation this skill produces. If it doesn't exist, proceed with a neutral professional tone and suggest running `content-brand-setup` once at the end.

## Phase 2: On-page citability fixes (the owned-content front)

Social builds narrative presence, but LLMs cite pages. For each missing or contested prompt cluster from the analysis above where the brand has a relevant existing page:

1. **Identify the best-matching page** (site search, or `DATA_getDomainKeywords` filtered by the cluster topic).
2. **Give passage-level recommendations that raise citability**: a direct 40–60-word answer near the top of the page; a datestamped stat with a primary source; entity disambiguation (who/what the brand is, stated plainly); question-mirroring H2s; and an extractable definition or list. For each page, name the prompt cluster it should win and explain why the current copy isn't being cited.
3. **Flag true content gaps.** Where no page exists for a missing cluster, mark it as a content gap with a one-line brief seed — `content-gap-sprint` or `content-campaign-brief` can take it from there.

## Phase 3: Before/after share-of-voice re-check

Record today's AI share-of-voice snapshot (the Phase 1 numbers: which engines, which prompts, who's cited). Set a re-check for 4–6 weeks out: same prompts, same engines, compare owned/contested/missing movement, and correlate with the social posts and on-page changes that shipped. Without the baseline, nobody can prove the work moved anything.

## Final output (extends the social campaign)

**AI-VISIBILITY-PLAN.md**: the SoV snapshot + prompt clusters from Phase 1, the per-page citability fixes, content-gap seeds, the social drafts (in Planable), and the re-check date with the exact prompt list to re-run.
